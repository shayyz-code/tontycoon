import { TTask } from "@context/ContextEarn"
import PrimaryButton from "../PrimaryButton"

type TTaskCard = {
  task: TTask
}

export default function TaskCard({ task }: TTaskCard) {
  return (
    <div className="relative flex flex-col items-between justify-end rounded-lg bg-indigo-950 ">
      {task.icon}
      <div className="px-7 py-5 flex justify-between items-center gap-5">
        <div>
          <h3 className="text-sm font-semibold mb-2">{task.tname}</h3>
          <div className="text-xs">+{task.points}pts</div>
        </div>
        <div>
          <PrimaryButton>Open</PrimaryButton>
        </div>
      </div>
    </div>
  )
}
